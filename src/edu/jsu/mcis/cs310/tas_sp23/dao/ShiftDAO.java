...
public class ShiftDAO { 
 ...
 ResultSet rs = null; try { Connection conn = daoFactory.getConnection();
    if (conn.isValid(0)) {
 ...
 shift = new Shift(id, description, shiftStart, shiftStop, roundInteral, gracePeriod, dockPenalty, lunchStart, lunchStop, lunchThreshold); } } } } catch (SQLException e) { throw new DAOException(e.getMessage()); } finally { if (rs != null) {
 try {
 rs.close();
